  ...
  const handleDirectLogin = async () => {
    ...
    try {
      ...
      const registerResponse = await fetch('https://web-production-84a3.up.railway.app/api/auth/register', {
        ...
        body: JSON.stringify({
          email: 'admin@direct.com',
          password: 'admin123',
          username: 'directadmin',
          business_name: 'Direct Business',
          business_reason: 'Direct access testing'
        }),
        credentials: 'include'
      });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ color: '#333', marginBottom: '20px' }}>🔐 Direct Login</h1>
        <p style={{ color: '#666', marginBottom: '30px' }}>One-click admin access</p>
        ...
        <p style={{ color: '#999', fontSize: '12px', marginTop: '20px' }}>
          This will register and login automatically with proper session management
        </p>
      </div>
    </div>
  );
