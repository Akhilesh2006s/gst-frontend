          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-xl font-medium transition-all duration-300 flex items-center space-x-2"
              >
                <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="backdrop-blur-xl bg-white/10 rounded-3xl shadow-2xl border border-white/20 p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            {/* Business Info */}
            <div>
              <h2 className="text-2xl font-bold text-white mb-4">{invoice.business_name}</h2>
            ...
            <div className="text-right">
              <h3 className="text-3xl font-bold text-white mb-2">INVOICE</h3>
              <div className="space-y-2 text-gray-300">
                <p><span className="font-semibold">Invoice #:</span> {invoice.invoice_number}</p>
                <p><span className="font-semibold">Date:</span> {formatDate(invoice.invoice_date)}</p>
          ...
          <div className="mb-8">
            ...
            <div className="backdrop-blur-xl bg-white/5 rounded-2xl p-6">
              <h4 className="text-lg font-semibold text-white mb-2">{invoice.customer_name}</h4>
          ...
          <div className="mb-8">
            ...
            <div className="backdrop-blur-xl bg-white/5 rounded-2xl overflow-hidden">
              <table className="w-full">
                <thead>
                  <tr className="bg-white/10">
                    <th className="px-6 py-4 text-left text-white font-semibold">Item</th>
                    <th className="px-6 py-4 text-center text-white font-semibold">Quantity</th>
                    <th className="px-6 py-4 text-right text-white font-semibold">Unit Price</th>
                    <th className="px-6 py-4 text-right text-white font-semibold">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {invoice.items.map((item, index) => (
                    <tr key={item.id} className={index % 2 === 0 ? 'bg-white/5' : 'bg-white/10'}>
                      <td className="px-6 py-4 text-white">{item.product_name}</td>
                      <td className="px-6 py-4 text-center text-gray-300">{item.quantity}</td>
                      <td className="px-6 py-4 text-right text-gray-300">{formatCurrency(item.unit_price)}</td>
                      <td className="px-6 py-4 text-right text-white font-semibold">{formatCurrency(item.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex justify-end mb-8">
            <div className="backdrop-blur-xl bg-white/5 rounded-2xl p-6 min-w-[300px]">
              <div className="text-right">
                <h3 className="text-2xl font-bold text-white mb-2">Total Amount</h3>
                <p className="text-3xl font-bold text-green-400">{formatCurrency(invoice.total_amount)}</p>
              </div>
            </div>
          </div>
